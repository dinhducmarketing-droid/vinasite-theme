...
function vinasite_handle_consultation()
{
    ...
    if ($name === '' || $phone === '' || !$consent) {
        vinasite_consultation_respond(false, 'Vui lòng nhập họ tên, số điện thoại và đồng ý chính sách bảo mật.', $is_ajax);
    }
    if (!preg_match('/^[0-9+\s.\-]{8,15}$/', $phone)) {
        vinasite_consultation_respond(false, 'Số điện thoại không hợp lệ. Vui lòng kiểm tra lại.', $is_ajax);
    }
    ...
    if (!empty($_FILES['vinasite_file']['name']) && empty($_FILES['vinasite_file']['error'])) {
        ...
        if (!in_array($ext, $allowed, true) || empty($check['ext'])) {
            vinasite_consultation_respond(false, 'Định dạng tệp không được hỗ trợ.', $is_ajax);
        }
        ...
        remove_filter('upload_dir', 'vinasite_private_upload_dir');
    ...
    $labels = array();
    foreach (vinasite_practice_areas() as $a) { $labels[$a['key']] = $a['title']; }
    if (function_exists('vinasite_home_services')) {
        foreach (vinasite_home_services() as $s) { $labels[$s['key']] = $s['title']; }
    }
    $area_label = isset($labels[$area]) ? $labels[$area] : ($area === 'khac' ? 'Chưa rõ / khác' : $area);

    $to      = vinasite_opt('form_email');
    ...
    if ($sent) {
        vinasite_consultation_respond(true, 'Cảm ơn bạn! Yêu cầu đã được gửi. Chúng tôi sẽ liên hệ lại trong thời gian sớm nhất.', $is_ajax);
    }
    vinasite_consultation_respond(false, 'Không gửi được yêu cầu. Vui lòng gọi ' . esc_html(vinasite_opt('phone')) . ' để được hỗ trợ ngay.', $is_ajax);
}
...
function vinasite_private_upload_dir($dirs)
{
    ...
    if (!file_exists($ht)) {
        ...
        // right after the email is sent (see vinasite_handle_consultation step 7b).
 ...
 */
function vinasite_obfuscate_filename($dir, $name, $ext)
{
